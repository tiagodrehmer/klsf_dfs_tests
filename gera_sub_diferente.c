...
// gcc -o p gera_sub_diferente.c -I gurobi912/linux64/include/ -L gurobi912/linux64/lib/ -lgurobi91 -lm
...
void load_infos(VNS* vns, int L, int k){

	int i;

	FILE *file_infos = fopen("arquivo_aux.txt", "r");
	...
	for(i = 0; i < k; i++){
		...
		fscanf(file_infos, "%d ", &vns->solution_now[i]);
	}

	for(i = 0; i < k; i++){
		fscanf(file_infos, "%d ", &vns->best_know[i]);
	}

	//int aux = 0;
	for(i = 0; i < L; i++){
		fscanf(file_infos, "%d ", &vns->ponderamento[i]);
		//aux += MAX_POND;
	}
	
	fscanf(file_infos, "%lf %lf %ld %d %d %d", &vns->best, &vns->solution_value, &vns->time, &vns->iteracao, &vns->it_in_solution, &vns->total_pond);

	//vns->total_pond = MAX_POND *;
...
VNS 
start_alg(int V, int L, int k, int flag){
	VNS vns;
...
double generate_model_and_solve(int qtd_labels_sub, Problem sub_h, double*solution){
	char name[100];
...
int main(int argc, char **argv){
	...
	if(error) exit(1);
	srand(atoi(argv[2]));
	int id_part = atoi(argv[3]);
	int max_exec = atoi(argv[4]);
	MAX_ITERATION = atoi(argv[5]); //1017
	MAX_IN_SOLUTION = atoi(argv[6]); //1017
	FRAC_FIX = atof(argv[7]); 
	FRAC_CORES_SUB_SAI = atof(argv[8]);
	FRAC_SUB_FICA = atof(argv[9]);
	MAX_POND = atoi(argv[10]);
	


	int sol_to_avaliation[G.L];
	vns = start_alg(G.V, G.L, G.k, id_part);
	...
	if(vns.best > 1 && (time(NULL) - vns.time) < 300){
		while(it < MAX_ITERATION && (time(NULL) - vns.time) < 300){
			...
			printf("\n");
			result = generate_model_and_solve(vns.qtd_labels_sub, sub, solution_solver);
			...
			printf("\n%s\nbest: %.0f\nit: %d \ntempo: %ld", argv[1], vns.best, vns.iteracao, time(NULL) - vns.time);
			finish_problem(&sub);
		}
