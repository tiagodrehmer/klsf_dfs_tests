...
// gcc -o p perfeitinho.c -I gurobi912/linux64/include/ -L gurobi912/linux64/lib/ -lgurobi91 -lm
...
double generate_model_and_solve(int qtd_labels_sub, Problem sub_h, double*solution, GRBenv *env){
	char name[100];
...
int main(int argc, char **argv){
	...
	if(vns.best > 1 && (time(NULL) - vns.time) < 300){
		while(it < MAX_ITERATION && (time(NULL) - vns.time) < 300){
			...
			//printf("\n");
			result = generate_model_and_solve(vns.qtd_labels_sub, sub, solution_solver, vns.env);
